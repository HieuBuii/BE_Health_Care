  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (
        ...
        if (appointment) {
          await mailServices.sendEmailCancelToCustomer({
            email: data.emailPatient,
            nameDoctor: `${data.lastNameDoctor} ${data.firstNameDoctor}`,
            fullNamePatient: `${data.lastNamePatient} ${data.firstNamePatient}`,
            dateAppointment: data.dateBooking,
            timeAppointment: data.timeBooking,
          });
