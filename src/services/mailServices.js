...
const buildContentsendEmailToCustomer = (dataSend) => {
  let content = `<h3>Xin chào ${dataSend.fullNamePatient}</h3>
    ...
    <ul>
        ...
        <li><b>Bác sĩ: ${dataSend.nameDoctor}.</b></li>
    </ul>
    </br>
...
const sendEmailCancelToCustomer = async (dataSend) => {
  // create reusable transporter object using the default SMTP transport
  let transporter = nodemailer.createTransport({
    host: "smtp.gmail.com",
    port: 587,
  ...
  let info = await transporter.sendMail({
    from: '"Health Care" <bt.hieuBVT145@gmail.com>', // sender address
    to: dataSend.email, // list of receivers
    subject: "Lịch hẹn khám bệnh đã bị huỷ ✔", // Subject line
    html: buildContentsendEmailCancelToCustomer(dataSend),
  });
};

const buildContentsendEmailCancelToCustomer = (dataSend) => {
  let content = `<h3>Xin chào ${dataSend.fullNamePatient}</h3>
    <p>Về lịch hẹn khám bệnh:</p>
    <ul>
        <li><b>Ngày: ${dataSend.dateAppointment}.</b></li>
    ...
    <p>Do bác sĩ có việc bận vào thời gian trên nên không thể tiếp nhận lịch hẹn của bạn.</p>
    <p>Rất mong bạn thông cảm và lựa chọn lịch khám khác.</p>
    <p>Chúng tôi trân thành xin lỗi và kính mong quý khách tiếp tục sử dụng dịch vụ của chúng tôi.</p>
`;
  return content;
};
